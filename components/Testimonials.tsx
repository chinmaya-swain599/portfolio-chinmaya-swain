    ...
    ">
      ...
      <div className="flex items-center gap-4 mt-auto">
        ...
        <div>
          ...
          <p className="text-[11px] font-medium text-gray-500 dark:text-gray-500 uppercase tracking-widest mb-1.5">
  ...
  React.useEffect(() => {
    ...
    const scroll = (time: number) => {
      if (!isPaused) {
        ...
        accumulatedScroll += (deltaTime * 40) / 1000; // 40px per second speed
        
        if (accumulatedScroll >= 1) {
          el.scrollLeft += Math.floor(accumulatedScroll);
          accumulatedScroll -= Math.floor(accumulatedScroll);
          
          // Seamless infinite loop trick: when we scroll past the first set, reset to 0
  ...
  return (
    <section className="relative z-10 w-full py-16 md:py-24 bg-white dark:bg-[#0a0d1a] transition-colors duration-500 overflow-hidden">
      ...
      >
        ...
        <div
          ref={scrollRef}
          className="flex items-stretch py-10 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] cursor-grab active:cursor-grabbing"
          style={{ width: '100%' }}
        >
          {looped.map((t, i) => (
            <div key={i} className="shrink-0 h-auto">
              <TestimonialCard t={t} />
            </div>
          ))}
        </div>
